<script lang="ts">
	...
	const selectedRows = $derived(
		files.filter((file) => selectedFileSet.has(file.code))
	);

	function getRowKey(file: DriveFileItem): string {
		return file.code;
	}

	function handleRowClick(file: DriveFileItem) {
		onfileclick?.(file);
	}

	function handleRowDblClick(file: DriveFileItem) {
		onfiledblclick?.(file);
	}

	function handleRowContextMenu(event: MouseEvent, file: DriveFileItem) {
...
>
	...
	{#snippet row({ row })}
		...
		>
			<div class="drive-file-list__name-content">
				<Icon
